...
void runTests() {
    ...
    int numVxs[] = {500,500,2000,2000};
    int numEdges[] = {1000,5000,5000,10000};
    int maxCap = 50;
        ...
        for (int j = 0; j < trials; j++) {
            ...
            if (testEdmondsKarp) {
                ...
                if (j == (trials - 1)) {
                    ...
                    std::cout<<"Edmonds-Karp:\t"<<result->maxFlow<<"\t"<<edKarpSeqTimes[i]<<std::endl;
            ...
            if (testDinics) {
                ...
                if (j == (trials - 1)) {
                    ...
                    std::cout<<"dinic's: \t"<<result->maxFlow<<"\t"<<dinicSeqTimes[i]<<std::endl;
            ...
            if (testPushRelabel) {
                ...
                if (j == (trials - 1)) {
                    ...
                    std::cout<<"push relable:\t"<<result->maxFlow<<"\t"<<pushRelabelSeqTimes[i]<<std::endl;
            ...
            if (testEdmondsKarp) {
                ...
                if (j == (trials - 1)) {
                    ...
                    std::cout << "Edmonds-Karp parallel:\t" << result->maxFlow<<"\t"<<edKarpCPUTimes[i]<<std::endl;
                    std::cout << "Avg speedup over edKarpSeq: " << (edKarpSeqTimes[i] / edKarpCPUTimes[i]) << std::endl << std::endl;
                }
                check = checkFlow(result->maxFlow, result->finalEdgeFlows, graphs[i]->n);
                if (!check) {
            ...
            if (testDinics) {
                ...
                if (i < numGraphs) {
                 ...
                 if (j == (trials - 1)) {
                    ...
                    std::cout << "dinic's parallel:\t" << result->maxFlow<<"\t"<<dinicCPUTimes[i]<<std::endl;
                    std::cout << "Avg speedup over dinicSeq: " << (dinicSeqTimes[i] / dinicCPUTimes[i]) << std::endl << std::endl;
                }
                check = checkFlow(result->maxFlow, result->finalEdgeFlows, graphs[i]->n);
                if (!check) {
            ...
            if (testPushRelabel) {
                ...
                if (j == (trials - 1)) {
                    ...
                    std::cout << "push relabel parallel:\t" << result->maxFlow<<"\t"<<pushRelabelCPULFTimes[i]<<std::endl;
                    std::cout << "Avg speedup over pushRelabelSeq: " << (pushRelabelSeqTimes[i] / pushRelabelCPULFTimes[i]) << std::endl << std::endl;
                }
                check = checkFlow(result->maxFlow, result->finalEdgeFlows, graphs[i]->n);
                if (!check) {
...
int main() {
    ...
    edges.push_back(std::make_tuple(0, 1, 7));
    edges.push_back(std::make_tuple(0, 2, 4));
    edges.push_back(std::make_tuple(1, 3, 5));
    edges.push_back(std::make_tuple(1, 4, 3));
    edges.push_back(std::make_tuple(2, 1, 3));
    edges.push_back(std::make_tuple(2, 4, 2));
    edges.push_back(std::make_tuple(3, 5, 8));
    edges.push_back(std::make_tuple(4, 3, 3));
    edges.push_back(std::make_tuple(4, 5, 5));
